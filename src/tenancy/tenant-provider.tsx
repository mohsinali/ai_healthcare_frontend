...
export function resolveTenantSelection(
  ...
  if (stored && memberships.some((item) => item.tenant.id === stored))
    return stored;
  return memberships.length === 1 ? memberships[0].tenant.id : null;
}
interface Value {
  currentMembership: TenantMembership | null;
  currentTenant: TenantMembership["tenant"] | null;
...
export function TenantProvider({ children }: { children: React.ReactNode }) {
  ...
  const memberships = useMemo(
    () =>
      (user?.tenantMemberships ?? []).filter(
        (item) => item.status === "ACTIVE" && item.tenant.status === "ACTIVE",
      ),
    [user],
  );
  ...
  useEffect(() => {
    ...
    const next = resolveTenantSelection(memberships, stored);
    // Membership refresh is authoritative; reconcile the convenience selection immediately.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setSelectedId(next);
    setInitializedFor(user?.id ?? "anonymous");
    if (next) window.localStorage.setItem(STORAGE_KEY, next);
    else window.localStorage.removeItem(STORAGE_KEY);
  }, [memberships, user?.id]);
  const selectTenant = useCallback(
    async (id: string) => {
      if (!memberships.some((item) => item.tenant.id === id)) return;
  ...
  const currentMembership =
    memberships.find((item) => item.tenant.id === selectedId) ?? null;
  const value = useMemo(
    () => ({
      currentMembership,
      ...
      isTenantSelected: Boolean(currentMembership),
      isInitializing: Boolean(user) && initializedFor !== user?.id,
      selectTenant,
    }),
    [currentMembership, initializedFor, memberships, selectTenant, user],
  );
  return (
