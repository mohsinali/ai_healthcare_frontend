...
export function FaqDetail({ faqId }: { faqId: string }) {
  ...
  if (query.isError) { const status = query.error instanceof ApiError ? query.error.status : 0; return <AppShell><State title={status === 404 ? "FAQ Not Found" : status === 403 ? "Access Denied" : "Unable to Load FAQ"} description={status === 404 ? "This FAQ does not exist or is not available in the current clinic." : status === 403 ? "You do not have permission to view this FAQ." : "Something went wrong while loading this Knowledge Base entry."} /></AppShell>; }
...
function State({ title, description }: { title: string; description: string }) { return <div className="p-8 text-center"><h1 className="text-xl font-semibold">{title}</h1><p className="mt-2 text-sm text-muted-foreground">{description}</p><Button asChild variant="outline" className="mt-5"><Link href="/knowledge-base">Back to Knowledge Base</Link></Button></div>; }
