...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const signIn = useCallback(async (email: string, password: string) => { const result = await apiRequest<{ accessToken: string; user: AuthUser }>("/auth/login", { method: "POST", body: JSON.stringify({ email, password }) }, false); updateToken(result.accessToken); setUser(await apiRequest<AuthUser>("/auth/me")); setStatus("authenticated"); }, [updateToken]);
